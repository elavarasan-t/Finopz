...
from src.controller import get_usage_daily

router = APIRouter(tags=["Azure/CostManagement/Usage"])

@router.post('/usageDaily')
@limiter.limit("50/minute")
async def usage_daily(Credential: Credentials, Data: CostRequest, request: Request, response: Response):
    try:
