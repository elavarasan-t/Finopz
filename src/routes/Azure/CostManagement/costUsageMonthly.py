...
from src.controller import get_azure_cost_usage_monthly

router = APIRouter(tags=["Azure/CostManagement/CostUsage"])

@router.post('/costUsageMonthly')
@limiter.limit("50/minute")
async def cost_usage_monthly(Credential: Credentials, Data: CostRequest, request: Request, response: Response):
    try:
        azure_auth = AzureAuth(Credential=Credential)
        credentials = azure_auth.authenticate()
        data = []
        cost_response = await run_in_threadpool(
            get_azure_cost_usage_monthly, 
            Data.scope, 
            credentials,
            Data.grouping,
            Data.cost_type,
